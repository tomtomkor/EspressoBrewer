...
- Ramping up: The pump power level gradually increases to the user-set max level(typically
  full power; 100%). You can try lower pressure extractions (e.g. Turbo shot) if you set the
  max level to be lower than 100%.
- Extraction: User-set max pump power is applied until you turn the extraction switch off.
- Back-to-Back: The coffee machine will warm up again. It will be 'Ready' after 2(not 5) minutes.
  This also can be changed in the arduino file.
...
- The parameters are: preinfusion duration(PreTime), preinfusion pump power(PrePower), 
  blooming duration(Pause), maximum pump power for extraction(MxPower), and
  ramp-up duration(RampUp)
...
- Once ESP32C3 gets the profile you sent from the phone, the received parameter values
